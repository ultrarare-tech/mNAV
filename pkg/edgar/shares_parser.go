...
func (p *SharesParser) ExtractSharesFromFiling(body []byte, filing Filing) (*SharesOutstandingRecord, error) {
...
func (p *SharesParser) extractFromHTML(body []byte, filing Filing) (*SharesOutstandingRecord, error) {
	doc, err := goquery.NewDocumentFromReader(bytes.NewReader(body))
	if err != nil {
		return nil, fmt.Errorf("error parsing HTML: %w", err)
	}

	var bestMatch *SharesOutstandingRecord
...
func (p *SharesParser) extractFromText(text string, filing Filing) (*SharesOutstandingRecord, error) {
	// Try each pattern
...
func (p *SharesParser) extractSharesFromText(text string, filing Filing, section string) *SharesOutstandingRecord {
	for patternName, pattern := range p.patterns {
		matches := pattern.FindAllStringSubmatch(text, -1)
		for _, match := range matches {
			if len(match) >= 2 {
...
func (p *SharesParser) extractSharesFromTable(table *goquery.Selection, filing Filing) *SharesOutstandingRecord {

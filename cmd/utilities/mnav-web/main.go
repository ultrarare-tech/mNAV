...
<body>
    ...
    <script>
        ...
        function renderData(data) {
            ...
            if (data.bitcoin_holdings || data.bitcoin_value) {
                html += '<div class="section">';
                html += '<h2 class="section-title">🪙 Bitcoin Holdings</h2>';
                html += '<table class="data-table">';
                html += '<thead><tr><th>Metric</th><th>Value</th></tr></thead>';
                html += '<tbody>';
                
                if (data.bitcoin_holdings) {
                    html += '<tr><td>Total BTC Holdings</td><td>' + data.bitcoin_holdings + ' BTC</td></tr>';
                }
                if (data.bitcoin_value) {
                    html += '<tr><td>Bitcoin Value</td><td>$' + data.bitcoin_value + '</td></tr>';
                }
                if (data.mnav) {
                    html += '<tr><td>mNAV per Share</td><td>$' + data.mnav + '</td></tr>';
                }
            ...
            if (data.portfolio && Object.keys(data.portfolio).length > 0) {
                ...
                html += '<table class="data-table">';
                html += '<thead><tr><th>Asset</th><th>Details</th></tr></thead>';
                html += '<tbody>';
            ...
            if (data.data_sources && Object.keys(data.data_sources).length > 0) {
                html += '<div class="section">';
                html += '<h2 class="section-title">📡 Data Sources</h2>';
                html += '<table class="data-table">';
                html += '<thead><tr><th>Source</th><th>Details</th></tr></thead>';
                html += '<tbody>';
                
                for (const [key, value] of Object.entries(data.data_sources)) {
                    html += '<tr><td>' + key + '</td><td>' + JSON.stringify(value) + '</td></tr>';
                }
...
// parseScriptOutput extracts key metrics from the script output
func (ws *WebServer) parseScriptOutput(result *ScriptOutput, output string) {
	...
	for _, line := range lines {
		...
		// Extract prices based on current section
		...
		if strings.Contains(line, "Bitcoin Holdings:") && strings.Contains(line, "BTC") {
			...
			for i, part := range parts {
				if strings.Contains(part, "BTC") && i > 0 {
					result.BitcoinHoldings = strings.Replace(parts[i-1], ",", "", -1)
					break
				}
			}
		}

		// Extract Bitcoin value
		if strings.Contains(line, "Bitcoin Value:") && strings.Contains(line, "$") {
			parts := strings.Split(line, "$")
			if len(parts) > 1 {
				// Remove "billion" or "million" and clean up
		...
		// Extract mNAV per share (if displayed differently)
